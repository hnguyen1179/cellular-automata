document.addEventListener("DOMContentLoaded", () => {
    ...
    const rows = 28;
    const cols = 350;
    const initialSize = Math.floor(350 * 0.75);
    const genTime = 0.05;
    ...
    const generateRandomInitial = (n) => {
        for (let i = 0; i < n; i++) rowA.add((Math.floor(Math.random() * 251)));
    }

    generateRandomInitial(initialSize);

    let svg = d3.select("#traffic-model-75")
        .attr("width", cellWidth * cols)
        .attr("height", cellHeight * rows);

    d3.select(".traffic-model-75-title").append('span')
        .classed("rule-set-random", true)
        .text(`Rule ${ruleSetNumber}`);

    const update = selection => {
        const row = selection.selectAll("#traffic-model-75").data(rowA, d => {
            return d[0];
        });
    ...
    let spacing = document.getElementsByClassName("traffic-model-75-spacing")[0];
